...
Z_WINDOW = 20
HORIZON = 5         # 5 handelsdagar framåt för facit
LOOKBACK = 60         # senaste 60 dagar (med facit) för ranking
BUY_TH = 0.3
SELL_TH = -0.3

# Trend-signal parametrar
MA_SHORT = 20
MA_LONG = 60
DONCHIAN_WINDOW = 20
TSMOM_WINDOW = 20
...
def rolling_regression_slope_logprice(price: pd.Series, window: int) -> pd.Series:
    # slope(log(price)) mot tid i [0..window-1]
...
def normalize_signals(out: pd.DataFrame, window: int) -> pd.DataFrame:
    ...
    for c in signal_cols:
        d[f"{c}_z{window}"] = rolling_zscore(pd.to_numeric(d[c], errors="coerce"), window=window)
    return d
...
def compute_weighted_recommendation(
    ...
    # 2) Vikter 5..1 (bäst->sämst)
    sorted_cols = sorted(correct_counts.items(), key=lambda x: x[1], reverse=True)
    weights = {col: 1 for i, (col, _) in enumerate(sorted_cols)}  # Här sker viktnignen. Vill bestämma en bra viktning ist för 5 - i och man lägger 1 om man vill ha likviktning  
